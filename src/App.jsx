  ...
  return (
    <div ref={overlayRef} onClick={e => e.target === overlayRef.current && onClose()}
      ...
      <style>{`
        ...
        .col-h{font-size:10px;font-weight:700;color:#B09080;letter-spacing:.5px;text-align:center}
        ...
        .line-total{font-size:11px;color:#8B4E2A;font-weight:700;text-align:center;background:#FDF0E8;border-radius:8px;padding:3px 6px;margin:-2px 0 8px 38px;display:inline-block}
              ...
              <div key={item.uid}>
                <div className="irow">
                  <input className="finp" placeholder={`المنتج ${idx+1}`} value={item.product}
                    onChange={e => updateItem(item.uid,"product",e.target.value)}
                    style={{padding:"10px 11px"}} />
                  <input className="finp" placeholder="١" type="number" min="1" value={item.qty}
                    onChange={e => updateItem(item.uid,"qty",e.target.value)}
                  ...
                  <input className="finp" placeholder="السعر" type="number" min="0" value={item.price}
                    ...
                    style={{padding:"10px 11px",textAlign:"center"}} />
...
export default function App() {
  ...
  // Real-time base cost (Private section)
  useEffect(() => {
    return onSnapshot(doc(db, "settings", "private"), snap => {
      if (snap.exists()) setBaseCost(snap.data().baseCost ?? "");
    });
  }, []);
  ...
  const saveBaseCost = async val => {
    ...
    await setDoc(doc(db, "settings", "private"), { baseCost: num });
  };
  ...
  const pending     = grandTotal - collected - transferred;

  const baseCostNum = parseFloat(baseCost) || 0;
  const profit      = baseCostNum > 0 ? grandTotal - baseCostNum : null;
  const profitPct   = profit !== null ? ((profit / baseCostNum) * 100).toFixed(1) : null;

  const catLabel = CATEGORIES.find(c => c.id === activeCategory)?.label || "";
  const statsCount = activeCategory === "private" ? 6 : 5;
    ...
    <div dir="rtl" style={{minHeight:"100vh",background:"#FDF8F4",fontFamily:"'Cairo',sans-serif",color:"#2C1A0E"}}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .stat-box{padding:18px 20px;border-left:1px solid #E8DDD3;transition:background .2s;min-width:0}
        .stat-box:last-child{border-left:none}
        .stat-box:hover{background:#FDF5EF}
        .slbl{font-size:10px;color:#B09080;font-weight:700;letter-spacing:1px;margin-bottom:5px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
        .sval{font-size:20px;font-weight:900;white-space:nowrap}
        ...
        .paid:hover{background:#B8E0C4}
        .unpaid:hover{background:#EDE0CC}
        .transferred:hover{background:#BFDBFE}
        ...
        .qty-badge{background:#F5EDE3;color:#8B4E2A;font-size:10px;font-weight:700;padding:2px 7px;border-radius:10px;margin-left:7px;white-space:nowrap}
        ...
        @media(max-width:700px){
          ...
          .cat-tab{font-size:12px;padding:12px 12px}
          .cright{gap:6px}
        }
      `}</style>
      ...
      <div className="stats-row" style={{gridTemplateColumns:`repeat(${statsCount},1fr)`}}>
        ...
        <div className="stat-box">
          <div className="slbl">تم التحويل</div>
          <div className="sval" style={{color:"#1A56A0"}}>{transferred.toLocaleString()} ج</div>
        </div>
        <div className="stat-box">
          ...
          <div className="stat-box" style={{cursor:"pointer"}} onClick={() => !editingBase && setEditingBase(true)}>
            ...
            {editingBase ? (
              ...
              <div>
                ...
                {profit !== null && (
                  <div style={{fontSize:12,fontWeight:700,marginTop:3,color: profit >= 0 ? "#276749" : "#CC3322"}}>
                    {profit >= 0 ? "▲" : "▼"} ربح: {Math.abs(profit).toLocaleString()} ج ({profitPct}%)
                  </div>
                )}
              </div>
            )}
      ...
      {/* ── Client Cards ── */}
      <div className="main">
        <div className="shead">تفاصيل الطلبات — {catLabel}</div>
        ...
        {!loading && catClients.length === 0 && (
          <div style={{textAlign:"center",padding:"60px",color:"#C9956E",fontSize:15}}>
            لا يوجد عملاء في هذا القسم — اضغط "إضافة عميلة جديدة" 💄
          </div>
        )}
            ...
            <div className="ccard" key={client.id}>
              <div className="chdr" onClick={() => setExpanded(isOpen ? null : client.id)}>
                ...
                <div className="cright" onClick={e => e.stopPropagation()}>
                  <button className={`spill ${st.cls}`} onClick={() => handleStatusToggle(client)} title="اضغط لتغيير الحالة">
                    {st.icon}  {st.label}
                  </button>
